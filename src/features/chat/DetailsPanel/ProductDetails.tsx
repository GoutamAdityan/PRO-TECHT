...
export const ProductDetails: React.FC<ProductDetailsProps> = ({ product }) => {
  ...
  return (
    <Card className="mb-4">
      <CardHeader>
        <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Product Details</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex items-center gap-4 mb-4">
          {product.imageUrl && (
            <img src={product.imageUrl} alt={product.name} className="w-20 h-20 rounded-md object-cover border" />
          )}
          <div>
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm mb-4">
          ...
          <div className="flex flex-col">
            ...
            <Badge variant={getWarrantyBadgeVariant(product.warrantyStatus)}>{product.warrantyStatus}</Badge>
          </div>
