        ...
        <ul className="space-y-3 flex-grow">
          {products.slice(-4).reverse().map((product, index) => (
            <li key={product.id} className={`flex items-center space-x-3 py-2 ${index < products.slice(-4).length - 1 ? 'border-b border-foreground/5' : ''}`}>
              ...
              <div>
                <p className="text-foreground/90 font-medium">{product.name} ({product.model})</p>
                <p className="text-foreground/70 text-sm">Warranty: {product.warrantyExpiry}</p>
              </div>
            </li>
          ))}
        </ul>
      ) : (
        <p className="text-foreground/70 flex-grow">No products registered yet. Register one to see it here!</p>
      )}
      <Button
