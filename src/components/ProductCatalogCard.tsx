      ...
      <Card variant="consumer" className="h-full flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div className="flex items-center space-x-2">
            ...
            <CardTitle className="text-base font-medium text-foreground/90">{product.name}</CardTitle>
          </div>
          <div className="text-xs text-muted-foreground">{product.modelNumber}</div>
        </CardHeader>
        <CardContent className="flex-grow p-6">
          <div className="text-2xl font-bold text-foreground mb-2">{product.warrantyLength}</div>
